...
def net(image, i_th, keep_prob, reuse = None):
    name = 'net_common' + str(i_th)
    with tf.variable_scope(name, reuse=True):
        conv1 = slim.conv2d(inputs = image, num_outputs = 32, kernel_size= [5, 5],
                            stride = 2, scope = 'conv1')

        pool1 = slim.max_pool2d(inputs=conv1, kernel_size=[2, 2], stride=2, padding='SAME', scope='pool1')

        conv2 = slim.conv2d(inputs = pool1, num_outputs = 64, kernel_size= [3, 3],
                            stride = 1, scope = 'conv2')
        conv3 = slim.conv2d(inputs = conv2, num_outputs = 64, kernel_size= [3, 3],
                            stride = 1, scope = 'conv3')

        pool2 = slim.max_pool2d(inputs=conv3, kernel_size=[2, 2], stride=2, padding='SAME', scope='pool2')

        conv4 = slim.conv2d(inputs = pool2, num_outputs = 64, kernel_size= [3, 3],
                            stride = 1, scope = 'conv4')

        conv_shape = conv4.get_shape().as_list()
        conv_reshape = tf.reshape(conv4, [-1 , 15, conv_shape[1], conv_shape[2], conv_shape[3]])
        feature_map = []
        for i in range(15):
            feature_map.append(conv_reshape[:, i, :, :, :])
        feature_maps = tf.concat(feature_map, 3)

        # full connect layer
        shp = feature_maps.get_shape()
        dim = shp[1].value * shp[2].value * shp[3].value
        feature_maps_flat = tf.reshape(feature_maps, [-1,dim])
        fc1 = slim.fully_connected(inputs=feature_maps_flat, num_outputs=512,activation_fn=tf.nn.relu,
                                   scope='fc1',reuse= True)
        fc1_drop = tf.nn.dropout(fc1, keep_prob)  # dropout layer

        # scope_weight = slim.get_variables(scope = tf.get_variable_scope())
        fc1_weight = tf.get_variable('fc1/weights')
        ...
        print_activations(feature_maps)
