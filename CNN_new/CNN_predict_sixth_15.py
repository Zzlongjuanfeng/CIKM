...
def net(image, i_th, keep_prob):
    ...
    with tf.variable_scope(name, reuse=True):
        ...
        fc1 = slim.fully_connected(inputs=pool5_flat, num_outputs=512,activation_fn=tf.nn.relu, scope='fc1')
        fc1_drop = tf.nn.dropout(fc1, keep_prob)  # dropout layer

        fc2 = slim.fully_connected(inputs=fc1_drop, num_outputs=10, activation_fn=tf.nn.relu, scope='fc2')
        if i_th == 1 :
            # tf.summary.histogram("fc1/weight", tf.get_variable('fc1/weights'))
            tf.summary.histogram("fc1/out", fc1)
            tf.summary.histogram("fc2/out", fc2)
...
def main(_):
    ...
    with tf.name_scope('input_reshape'):
        ...
        x_image_h1 = x_image[:, :, :, 10:14, 1]
        x_image_h0 = x_image[:, :, :, 10:14, 0]
        x_image_h2 = x_image[:, :, :, 10:14, 2]
        x_image_h3 = x_image[:, :, :, 10:14, 3]
    ...
    # learning_rate = tf.train.exponential_decay(5e-4, global_step, 89, 0.5, staircase=True)
